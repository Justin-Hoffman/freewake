...
SimulationManager::SimulationManager() : surfaces_(), nOffset_(1,0), a(NULL), b(NULL), globalLinearVelocity_(), globalRotationAxis_(0.0,0.0,1.0), globalRotationRate_(0.0), netForce_(0.0, 0.0, 0.0), netMoment_(0.0, 0.0, 0.0)  {

}

SimulationManager::SimulationManager( SimulationManager& s ) : surfaces_(s.surfaces_), nOffset_(s.nOffset_), a(NULL), b(NULL), globalLinearVelocity_( s.globalLinearVelocity_), globalRotationAxis_( s.globalRotationAxis_ ), globalRotationRate_( s.globalRotationRate_),
                                                               netForce_( s.netForce_ ), netMoment_( s.netMoment_ ) {
...
void SimulationManager::solve(){
    ...
    /* Local arrays */
    int* ipiv = (MKL_INT*) malloc( maxN*sizeof(int) );
    ...
    double Cl =  netLift()/(1.0/2.0 *100.0*100.0 *5.0 );
    double Cd =  netDrag()/(1.0/2.0 *100.0*100.0 *5.0 );
    double CdIdeal = Cl*Cl/ (M_PI * 5.0 );
    ...
    printf("forces\n");
    netForce_.printState();
    free(a); free(b);
}

 
void printMatrix( char const* desc, MKL_INT m, MKL_INT n, double* a, MKL_INT lda ) {
        MKL_INT i, j;
        printf( "\n %s\n", desc );
        for( i = 0; i < m; i++ ) {
...
void print_int_vector( char const* desc, MKL_INT n, MKL_INT* a ) {
        MKL_INT j;
        printf( "\n %s\n", desc );
...
void SimulationManager::integrateForceAndMoment(){
    ...
    for( int h = 0; h < (int) surfaces_.size(); h++ ){
        ...
        for(int i = 0; i < s->nSpan(); i++){
            for(int j = 0; j < s->nChord(); j++){
                ...
                printf("Net Velocities:\n");
                vInf.printState();
                (vInf+vInduced).printState();
    ...
    netForce_ = netForce;
    netMoment_ = netMoment;
}

double SimulationManager::netDrag(){
    return netForce_.dot( globalLinearVelocity_.norm() ); 
}

double SimulationManager::netLift(){
    return netForce_.dot( globalLinearVelocity_.rotate(Vec3D(), Vec3D( 1.0, 0.0, 0.0 ), -M_PI / 2.0 ).norm() );
}


Vec3D SimulationManager::vInfinity( Vec3D p ){
    Vec3D vInf = globalLinearVelocity_;   
    //Vec3D vInf += Work out rotation contribution
    return vInf;
}
