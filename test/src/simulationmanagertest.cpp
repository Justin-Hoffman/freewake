...
TEST_F(SimulationManagerTest, TestFreeWake){
    ...
    // Result when vortex core size is small
    //double gamma[10] = { 31.041131485234366, 38.585766297136061, 41.89244078663905, 43.521555625784941, 44.215426954414284, 44.215426954414284, 43.521555625784941, 41.89244078663905, 38.585766297136061, 31.041131485234366};  
    //Result when vortex core size is larger
    double gamma[10] = { 35.03083712394531091, 43.13601948057333857, 46.29353105077682073, 47.71129676896759975, 48.28231926134120044, 48.28231926134120044, 47.71129676896760685, 46.29353105077682073, 43.13601948057334567, 35.03083712394531091};
    int n = -1;
    for(int i = 0; i < ls.nSpan(); i++){
        for(int j = 0; j < ls.nChord(); j++){
            n++;
            EXPECT_NEAR( gamma[n], ls.getHorseshoeLattice().getGamma()[i][j], 2E-0);
        }
    }
}
